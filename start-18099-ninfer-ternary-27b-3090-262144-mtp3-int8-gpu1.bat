@echo off
rem Ternary-Bonsai-2-27B-Abliterated (ninfer sm_86 build) - 262144 ctx + MTP head, GPU1 single card.
...
rem window and serve-18099-mtp3.log. Double-click this .bat and the log scrolls live.
...
echo   live log scrolls below; same lines also go to serve-18099-mtp3.log
echo  ==========================================================
echo.

"C:\Users\Administrator\AppData\Local\Programs\Python\Python314\python.exe" "H:\ninfer-ternary\tee_engine.py" "H:\ninfer-ternary\serve-18099-mtp3.log" -- "build\b1\apps\ninfer-serve.exe" "models\Ternary-Bonsai-2-27B-Abliterated-PQ2_0.ninfer" --host 0.0.0.0 --port 18099 --device 1 --max-context 262144 --kv-capacity 262144 --kv-dtype int8 --max-concurrency 2 --spec mtp --draft-tokens 3 --lm-head-draft --cors --model-id Bonsai2-27B-Abliterated-PQ2

echo.
echo [engine exited]  full log: H:\ninfer-ternary\serve-18099-mtp3.log
pause
